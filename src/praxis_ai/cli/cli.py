...
from praxis_ai.cli.repl import InteractiveREPL
from praxis_ai.cli.ui.renderer import TerminalRenderer
...
app = typer.Typer(
    name="praxis",
    help="Praxis AI: Autonomous Enterprise Operations & SOP-Grounded Agent Platform",
    add_completion=True,
...
def start_cmd(
    ...
    """Start an interactive Praxis AI operational assistant REPL."""
    start_interactive_session(model=model, permission=permission, workspace=workspace)


@app.command(name="run")
def run_cmd(
    prompt: str = typer.Argument(..., help="The operational instruction, query, or task to execute"),
    ...
    """Execute a single operational task, calculation, or prompt non-interactively."""
    settings = Settings.load()
    if model:
        settings.model = resolve_model_name(model)
    settings.permission_mode = permission
...
def config_cmd() -> None:
    ...
    settings = Settings.load()
    table = Table(title="Praxis AI Configuration", border_style="cyan")
